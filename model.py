...
class highwayNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        # [수정 1] Neighbor Input Dimension 가져오기 (기본값 2)
        self.nbr_input_dim = args.get('nbr_input_dim', 2)

        ## Define network weights

        # [수정 2] Input embedding layer 분리
        # Ego는 항상 (x, y) 2차원
        self.ip_emb_ego = torch.nn.Linear(2, self.input_embedding_size)
        # Neighbor는 실험 모드에 따라 가변 (2, 5, 7 등)
        self.ip_emb_nbr = torch.nn.Linear(self.nbr_input_dim, self.input_embedding_size)
    ...
    def forward(self, hist, nbrs, masks, lat_enc, lon_enc):
        ...
        ## [수정 3] Forward pass hist: Ego용 Embedding 사용
        # hist shape: (T, Batch, 2)
        _, (hist_enc, _) = self.enc_lstm(self.leaky_relu(self.ip_emb_ego(hist)))
